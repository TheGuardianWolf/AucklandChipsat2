...
class MLX90640:
    def __init__(bus, address=0x33):  # default 0x33
        self.bus = bus
    ...
    def getRegs(self, reg, num):
        write_data = bytearray([reg >> 8, reg & 0xFF])
        read_data = bytearray(num)
        write = i2c_msg.write(self.addr, [reg >> 8, reg & 0xFF])
        read = i2c_msg.read(self.address, num)
        
        self.bus.writeto(self.address, write_data)
        self.bus.readfrom_into(self.address, read_data)
        return list(read)

    def getRegf(self, reg):
        write_data = bytearray([reg >> 8, reg & 0xFF])
        read_data = bytearray(2)
        self.bus.writeto(self.address, write_data)
        self.bus.readfrom_into(self.address, len(read_data))
    ...
    def getTa(self):
        ...
        Ta = (
            (Vptatart / float(1 + KVptat * self.DV) - Vptat25) / float(KTptat)
        ) + self.Ta0
        return Ta
    ...
    def getPixData(self, i, j):
        ...
        pixOffset = (
            Offsetavg
            + OCCRow * pow(2, OCCscaleRow)
            + OCCCol * pow(2, OCCscaleCol)
    ...
    def getCompensatedPixData(self, i, j):
        ...
        alpha = (
            ...
            + (ACCRow << ACCScaleRow)
            + (ACCCol << ACCScaleCol)
            + (alphaPixel << ACCScaleRem)
        ) / float(pow(2, alphaScale))
        ...
        Sx = self.KsTo2 * self.root4(
            pow(alphacomp, 3) * VIRcomp + pow(alphacomp, 4) * Tar
        )

        return (
            self.root4((VIRcomp / (alphacomp * (1 - self.KsTo2 * 273.15) + Sx)) + Tar) - 273.15
        )
